...
<header id="item-header-content" class="entry-header member-single-header <?php if ( bp_is_my_profile() ) : echo 'my-profile'; endif; ?> clearfix">
	...
	<div id="item-header-avatar">
		...
		<p class="uname"><span class="at-mention">@<?php bp_displayed_user_username() ?></span></p>
		...
		<div class="item-buttons clearfix">
			...
			<?php if ( is_user_logged_in() && !bp_is_my_profile() && function_exists( 'bp_send_private_message_link' ) ) : ?>
				<div class="generic-button" id="send-private-message">
					<a href="<?php bp_send_private_message_link() ?>" title="<?php _e( 'Send a private message to this user.', 'buddypress' ) ?>"><?php _e( 'Send a note', 'buddypress' ) ?></a>
				</div>
			<?php endif; ?>
		</div><!-- #item-buttons -->
	...
	<h1 class="entry-title"><a href="<?php echo bp_loggedin_user_domain( '/' ) ?>"><?php bp_displayed_user_fullname() ?></a></h1>

	<p class="bio"> <?php bp_profile_field_data( 'field=About Me' ); ?> </p>
</header>
