...
buildmenu:function($, setting){
	...
	$headers.each(function(i){ //loop through each LI header
		...
		$subul.data('timers', {})
		this._dimensions={w:this.offsetWidth, h:this.offsetHeight, subulw:$subul.width(), subulh:$subul.outerHeight()}
		...
		$curobj.hover(
			function(e){
				...
				header._dimensions={w:header.offsetWidth, h:header.offsetHeight, subulw:$subul.width(), subulh:$subul.outerHeight()}
				$subul.css({top:header.istopheader && setting.orientation!='v'? header._dimensions.h+"px" : 0})
				...
				$targetul.data('timers').showtimer=setTimeout(function(){
					...
					if ($targetul.queue().length<=1){ //if 1 or less queued animations
						// Responsive menus hack
						if(jQuery('#MM_responsive_hide').css('display') != 'inline-block' && jQuery('#MainMenu').hasClass('touching') == false) {
							$targetul.css({left:menuleft+"px", width:header._dimensions.subulw+'px'}).animate({height:'show',opacity:'show'}, ddsmoothmenu.transition.overtime)
						} else if ( jQuery('#MainMenu').hasClass('touching') ) {
							$targetul.css({left:'0', width:header._dimensions.subulw+'px'}).animate({height:'show',opacity:'show'}, ddsmoothmenu.transition.overtime)
						} else {
							$targetul.animate({height:'show',opacity:'show'}, ddsmoothmenu.transition.overtime)
						}
